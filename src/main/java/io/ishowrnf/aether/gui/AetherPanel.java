...
public class AetherPanel {
    ...
    private final List<AetherComponent> children = new ArrayList<>();
    ...
    public void add(AetherComponent c) {
        children.add(c);
    }
    ...
    public void render(int mouseX, int mouseY, float partialTicks) {
        ...
        int cy = y + 22 - scroll;
        contentHeight = 0;
        for (AetherComponent c : children) {
            c.setPosition(cx, cy);
            c.setSize(width - 12, c.height);
            // Simple vertical clipping: only render if within panel content area
    ...
    public void mouseReleased(int mouseX, int mouseY, int button) {
        ...
        for (AetherComponent c : children) c.mouseReleased(mouseX, mouseY, button);
    }
    ...
    public void mouseDragged(int mouseX, int mouseY, int button, long time) {
        if (dragging) {
            ...
            for (AetherComponent c : children) c.mouseDragged(mouseX, mouseY, button, time);
        }
    }
