...
/**
 * Minimal color picker component.
 * - Shows preset swatches
 * - Simple alpha slider
 *
 * This is intentionally small; a full SV square + hue wheel is left for later polish.
 */
public class AetherColorPicker extends AetherComponent {
    ...
    public void render(int mouseX, int mouseY, float partialTicks) {
        ...
        if (open) {
            ...
            for (int i = 0; i < PRESETS.size(); ++i) {
                ...
                if (containsPoint(mouseX, mouseY) && mouseX >= sx && mouseX <= sx + sw && mouseY >= boxY && mouseY <= boxY + sw) {
                    // highlight
                    Gui.drawRect(sx-1, boxY-1, sx+sw+1, boxY+sw+1, 0x40FFFFFF);
                }
            }
        }
    }
    ...
    public void mouseClicked(int mouseX, int mouseY, int button) {
        if (!open && containsPoint(mouseX, mouseY)) {
            open = true; return;
        }
        if (open) {
            int boxY = y + height + 4;
