...
import net.lax1dude.eaglercraft.v1_12.aether.gui.components.AetherToggle;
import net.lax1dude.eaglercraft.v1_12.aether.gui.components.AetherSlider;
...
public class AetherPanelManager {
    ...
    public AetherPanelManager(int screenWidth, int screenHeight) { buildDefaultPanels(screenWidth, screenHeight); }

    protected void buildDefaultPanels(int w, int h) {
        int cols = 4; int gap = 12; int panelWidth = (w - (cols + 1) * gap) / cols; int panelHeight = (int)(h * 0.55f); int startY = 20 + 48 + 12;
        ...
        for (int i = 0; i < cols; ++i) {
            ...
            for (int r = 0; r < 8; ++r) {
                ...
                p.add(t);
                if (r % 3 == 0) { AetherSlider s = new AetherSlider(fr, "Value", 0f, 100f, 50f); p.add(s); }
            }
            panels.add(p);
        }
    }
    ...
    public FontRenderer getFontRenderer() { return fr; }

    public void render(int mouseX, int mouseY, float partialTicks) { for (AetherPanel p : panels) p.render(mouseX, mouseY, partialTicks); }
    public void mouseClicked(int mouseX, int mouseY, int button) { for (AetherPanel p : panels) p.mouseClicked(mouseX, mouseY, button); }
    public void mouseReleased(int mouseX, int mouseY, int button) { for (AetherPanel p : panels) p.mouseReleased(mouseX, mouseY, button); }
